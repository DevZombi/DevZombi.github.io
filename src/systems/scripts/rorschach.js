 ...
 */
function renderRorschachInkBlot (canvas,w,h,locX,locY, {
    blots = randInt(10, 100),
    iterations = 50
  } = {}) {
    const width = w * 0.525; // A little bigger than half the width so the mirroring looks good
    const height = h;
    ...
    for (let y = locY; y < canvas.height; y++) {
      for (let x = locX; x < canvas.width / 2; x++) {
        const value = get(x, y);
        ctx.fillStyle = `rgba(255, 255, 255, ${value / iterations})`;
